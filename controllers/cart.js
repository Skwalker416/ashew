...
exports.getUserCart = async function(req, res) {
    try {
        ...
        const cartEvents = await CartEvent.find({ _id: { $in: user.cart } });
        if (!cartEvents) {
            return res.status(404).json({ message: 'Cart not found' });
        }
        const cart = [];
        ...
        for (const cartEvent of cartEvents) {
            const event = await Event.findById(cartEvent.event);
            if (!event) {
                cart.push({
                    ...cartEvent._doc,
                    eventExists: false,
                    eventOutOfStock: false,
                });

            } else {
                cartEvent.eventName = product.name;
                cartEvent.eventImage = product.image;
                cartEvent.eventPrice = product.price;
                if (event.countInStock < cartEvent.quantity) {
                    cart.push({
                        ...cartEvent._doc,
                        eventExists: true,
                        eventOutOfStock: true,
                    });
                } else {
                    cart.push({
                        ...cartEvent._doc,
...
exports.getCartEventById = async function(req, res) {
    try {
        const cartEvent = await CartEvent.findById(req.params.cartEventId);
        if (!cartEvent) {
            return res.status(404).json({ message: 'Cart Event ticket not found!' });
        }
        ...
        if (!event) {
            ...
            cartEvent.eventName = event.name;
            cartEvent.eventImage = event.image;
            cartEvent.eventPrice = event.price;
            if (event.countInStock < cartEvent.quantity) {
                cart.push({
                    ...cartEvent._doc,
                    eventExists: true,
    ...
    try {
        ...
        if (existingCartItem) {
            ...
            if (condition) {
                ...
                await Product.findOneAndUpdate({ _id: productId }, { $inc: { countInStock: -1 } }).session(session);
        ...
        const updatedProduct = await Product.findOneAndUpdate({ _id: productId, countInStock: { $gte: cartProduct.quantity } }, { $inc: { countInStock: -cartProduct.quantity } }, { new: true, session });
...
exports.modifyEventQuantity = async function(req, res) {
    try {
        const user = await User.findById(req.params.id);
        if (!user) return res.status(404).json({ message: 'User not found' });
        ...
        let cartEvent = await CartEvent.findById(req.params.cartEventId);
        if (!cartEvent) {
            return res.status(404).json({ message: 'Event ticket not found' });
        }

        const actualEvent = await Event.findById(cartEvent.event);
        if (!actualEvent) {
            return res.status(404).json({ message: 'Event ticket does not exist' });
        }

        if (quantity > actualEvent.countInStock) {
            return res
                .status(400)
                .json({ message: 'Insufficient stock for the requested quantity' });
        }

        cartEvent = await CartEvent.findByIdAndUpdate(
            req.params.cartEventId,
            quantity, { new: true }
        );
...
exports.removeFromCart = async function(req, res) {
    ...
    try {
        ...
        if (!user.cart.includes(req.params.cartEventId)) {
            await session.abortTransaction();
            return res.status(400).json({ message: 'Event ticket not in your cart' });
        }

        // const cartItemToRemove = await CartProduct.findById(
        //     req.params.cartProductId
        // );
        // const cartItemToRemove = await CartEvent.findById(
        //     req.params.cartEventId
        // );
        // if (!cartItemToRemove) {
        //     await session.abortTransaction();
        ...
        // }
        const cartItemToRemove = await Promise.all([
            CartProduct.findById(req.params.cartProductId),
            CartEvent.findById(req.params.cartEventId)
        ]);

        if (!cartItemToRemove[0] || !cartItemToRemove[1]) {
            await session.abortTransaction();
            return res.status(404).json({ message: 'Cart item not found' });

        }
        if (cartItemToRemove.reserved) {
            const updatedProduct = await Product.findOneAndUpdate({ _id: cartItemToRemove.product }, { $inc: { countInStock: cartItemToRemove.quantity } }, { new: true, session });
            if (!updatedProduct) {
                await session.abortTransaction();
                return res.status(500).json({ message: 'Internal Server Error' });
            }
            const updatedEvent = await Event.findOneAndUpdate({ _id: cartItemToRemove.event }, { $inc: { countInStock: cartItemToRemove.quantity } }, { new: true, session });
            if (!updatedEvent) {
                await session.abortTransaction();
                return res.status(500).json({ message: 'Internal Server Error' });
            }
        ...
        const cartEvent = await CartEvent.findByIdAndDelete(
            cartItemToRemove.id
        ).session(session);
